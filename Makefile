...
jobs/step3/obs-%.jobs:
	...
	@for qtl_data in $(QTL_DATA) ; do zcat stat/IGAP/ld/$${qtl_data}/$*.obs_ld.gz | awk -vchr=$* -vgwas=IGAP -vqtl=$${qtl_data} '$$(NF) >= 100 { print "./make.mediation.type1.R" OFS ("stat/" gwas "/ld/" qtl "/" chr ".obs_ld.gz") OFS ("stat/" gwas "/" qtl "/" chr ".obs_bed.gz") FS ("1kg/chr" chr) FS NR FS 74000 FS 400 FS ("zqtl.type1/IGAP/1kg/" qtl "/" chr "/" NR ".type1.txt.gz") }' | awk 'system("[ ! -f " $$NF " ]") == 0' >> $@; done
	@for qtl_data in $(QTL_DATA) ; do zcat stat/IGAP/ld/$${qtl_data}/$*.obs_ld.gz | awk -vchr=$* -vgwas=IGAP -vqtl=$${qtl_data} '$$(NF) >= 100 { print "./make.mediation.type1.R" OFS ("stat/" gwas "/ld/" qtl "/" chr ".obs_ld.gz") OFS ("stat/" gwas "/" qtl "/" chr ".obs_bed.gz") FS ("geno/rosmap1709-chr" chr) FS NR FS 74000 FS 400 FS ("zqtl.type1/IGAP/rosmap/" qtl "/" chr "/" NR ".type1.txt.gz") }' | awk 'system("[ ! -f " $$NF " ]") == 0' >> $@; done

	@for qtl_data in $(QTL_DATA) ; do zcat stat/IGAP/ld/$${qtl_data}/$*.obs_ld.gz | awk -vchr=$* -vgwas=IGAP -vqtl=$${qtl_data} '$$(NF) >= 100 { print "./make.mediation.R" OFS ("stat/" gwas "/ld/" qtl "/" chr ".obs_ld.gz") OFS ("stat/" gwas "/" qtl "/" chr ".obs_bed.gz") FS ("1kg/chr" chr) FS NR FS 74000 FS 400 FS ("zqtl/IGAP/1kg/" qtl "/" chr "/" NR) }' | awk 'system("[ ! -f " $$NF ".zqtl.txt.gz ]") == 0' >> $@; done
	@for qtl_data in $(QTL_DATA) ; do zcat stat/IGAP/ld/$${qtl_data}/$*.obs_ld.gz | awk -vchr=$* -vgwas=IGAP -vqtl=$${qtl_data} '$$(NF) >= 100 { print "./make.mediation.R" OFS ("stat/" gwas "/ld/" qtl "/" chr ".obs_ld.gz") OFS ("stat/" gwas "/" qtl "/" chr ".obs_bed.gz") FS ("geno/rosmap1709-chr" chr) FS NR FS 74000 FS 400 FS ("zqtl/IGAP/rosmap/" qtl "/" chr "/" NR) }' | awk 'system("[ ! -f " $$NF ".zqtl.txt.gz ]") == 0' >> $@; done
