...
gene.tab.file <- 'tables/genes.txt.gz'
gene.tab.sig.file <- 'tables/genes_significant.txt.gz'
gene.tab.strict.file <- 'tables/genes_strict.txt.gz'
...
gene.tab.strict <- read_tsv(gene.tab.strict.file, col_names = TRUE) %>% na.omit() %>%
    group_by(hgnc) %>% slice(which.max(lodds))
...
df.sig <- df %>% filter(ensg %in% gene.tab.sig$ensg)
...
pve.pip.plot <- function() {
    ...
    ret <- ret + scale_x_continuous(limits = c(1e-3, 10), breaks = c(1e-2, .5, .99), trans = .logit) +
...
ggsave(filename = 'figures/gene_gwas_pip.pdf', plot = gwas.pip.plot(), width = 6, height = 4)
ggsave(filename = 'figures/gene_pip_gwas.pdf', plot = gwas.pip.plot(TRUE), width = 6, height = 4)

ggsave(filename = 'figures/gene_pve_pip.pdf', plot = pve.pip.plot(), width = 6, height = 4)

ggsave(filename = 'figures/gene_gwas_pve.pdf', plot = gwas.pve.plot(), width = 6, height = 4)
ggsave(filename = 'figures/gene_pve_gwas.pdf', plot = gwas.pve.plot(TRUE), width = 6, height = 4)
...
df.strict.neg <- df.sig %>% filter(abs(best.gwas.z) > 4, theta < 0)

df.strict.pos <- df.sig %>% filter(abs(best.gwas.z) > 4, theta > 0)
