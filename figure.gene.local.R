...
plt.med <- gg.plot(model.tab) +
    ...
    geom_segment(aes(x = (tss + tes)/2, xend = (tss + tes)/2, y = 0, yend = med.p),
                 color = 'red', arrow = arrow(length = unit(.05, 'inches')),
                 size = 1) +
    scale_y_reverse() + .gwas.x.scale + ylab('-log10 mediation p-value') +
